...
- **`core_di`** is a **barrel** that re-exports providers from `core_analytics`, `core_auth`, `cloud`, `core_env`, and `core_network` (including connectivity types). Feature or shell code that wants a single import for cross-cutting providers can depend on `core_di`.
...
- **Cloud** (`cloud` package): `firebaseDatabaseProvider`, `supabaseDatabaseProvider`, `neonDatabaseProvider`, and storage counterparts are **throwing placeholders** until the shell overrides them in `ProviderScope` when you wire a backend.
