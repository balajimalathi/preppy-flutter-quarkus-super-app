...
Feature packages today are mostly **screens and routes**; for new CRUD work, grow each feature with `domain/`, `data/`, `application/`, and `presentation/` under `lib/src/`. Shared entities live in [`core_models`](/packages/core-models/); HTTP, auth, and env stay in **`core_*`** (see [`core_auth`](/packages/core-auth/) for the cross-cutting pattern).

For a hands-on Syllabus example, see **[Example CRUD application](/guides/example-crud-application/)**. For layer map, decision tree, and checklists, see **[Clean architecture & CRUD](/guides/clean-architecture-crud/)**.
